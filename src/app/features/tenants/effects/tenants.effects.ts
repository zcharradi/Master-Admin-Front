...
// Type local étendu, en attendant que le client swagger soit régénéré
// pour inclure dbInstanceName et dbName renvoyés par l'endpoint GetAll
interface ErpTenantDTOWithDb extends ErpTenantDTO {
...
export class TenantsEffects {
  ...
  private mapDto(dto: ErpTenantDTOWithDb): Tenant {
    const dbLabel = dto.dbInstanceName && dto.dbName
      ? `${dto.dbInstanceName}-${dto.dbName}`
      : "";
  ...
  private mapList(result: any): ApiListResponse<Tenant> {
    const dtos: ErpTenantDTOWithDb[] = result?.data ?? result ?? [];
    return { data: dtos.map((d) => this.mapDto(d)), total: result?.total ?? dtos.length };
  }
}
