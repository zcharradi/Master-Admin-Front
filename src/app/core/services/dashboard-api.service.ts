...
export class DashboardApiService {
  ...
  load(): Observable<DashboardMetrics> {
    ...
    const tenants$ = this.tenantsApi.erpTenantGetAllPost(request);
    const dbs$ = this.dbApi.dbInstanceGetAllPost(request);
    const users$ = this.usersApi.erpUserGetAllPost(request);
